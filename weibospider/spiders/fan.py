...
class FanSpider(Spider):
    ...
    def start_requests(self):
        ...
        for user_id in user_ids:
            url = f"https://weibo.com/ajax/friendships/friends?relate=fans&page=1&uid={user_id}&type=fans"
    ...
    def parse(self, response, **kwargs):
        ...
        for user in data['users']:
            item = dict()
            item['follower_id'] = response.meta['user_id']
            item['fan_info'] = parse_user_info(user)
            item['_id'] = response.meta['user_id'] + '_' + item['fan_info']['_id']
            yield item
        if data['users']:
